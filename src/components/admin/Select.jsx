...
export default function Select({ className = '', ...props }) {
  return (
    <div className="relative inline-flex">
      <select
        className={`h-9 w-full appearance-none rounded-md border border-[var(--line)] bg-[var(--input-bg)] py-0 pl-3 pr-8 text-[13px] text-[var(--text)] outline-none transition-shadow duration-150 focus:border-[var(--green-800)] focus:ring-2 focus:ring-[var(--green-800)]/20 ${className}`.trim()}
        {...props}
      />
